...
import de.fhws.fiw.fds.demo.server.api.queries.UniversityNameQuery;
...
public class JerseyUni extends AbstractJerseyService
{
    ...
    public Response getAllUniversities(
            ...
            @DefaultValue("") @QueryParam("uni_location") final String uni_location,
            @DefaultValue("20") @QueryParam("size") int size) {
        try {
            return new GetAllUniversities(
                    this.serviceContext,
                    new UniversityNameQuery<>(universityName, order, offset, size)
            ).execute();
    ...
    @DefaultValue("") @QueryParam("name_of_module") final String name_of_module,
    @DefaultValue("0") @QueryParam("offset") int offset,
    @DefaultValue("20") @QueryParam("size") int size) {
        try {
            return new GetAllUniModule(this.serviceContext, personId, new ModuleQuery<>(personId, name_of_module, offset, size)).execute();
        } catch (SuttonWebAppException e) {
            throw new WebApplicationException(Response.status(e.getStatus().getCode())
                    .entity(e.getExceptionMessage()).build());
        }

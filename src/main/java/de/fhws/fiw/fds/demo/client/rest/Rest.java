...
public class Rest extends AbstractRestClient {
    ...
    private static final String FILTER_BY_NAME = "filterByName";
    private static final String FILTER_BY_NAME_REVERSED = "filterByNameReversed";
    ...
    public boolean isGetByNameAllowed() {
        return isLinkAvailable(FILTER_BY_NAME);
    }

    public boolean isGetByNameReversedAllowed() {
        return isLinkAvailable(FILTER_BY_NAME_REVERSED);
    }

    public void getByName(String name) throws IOException {
        if (isGetByNameAllowed()) {
    ...
    public void getByNameReversed(String name, String uni_location) throws IOException {
        if (isGetByNameReversedAllowed()) {
            var url = getUrl(FILTER_BY_NAME_REVERSED);
            url = url.replace("{UNIVERSITYNAME}", name);
            processResponse(this.universityWebClient.getCollectionOfUniversities(url), (response) -> {
                this.currentUniversityData = new LinkedList<>(response.getResponseData());
                this.cursorUniversityData = 0;
